...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
            <table class="table table-bordered table-striped {{ count($invitations) > 0 ? 'datatable' : '' }} @can('invitation_delete') dt-select @endcan">
                <thead>
                    <tr>
                        ...
                        @foreach ($invitations as $invitation)
                            <tr data-entry-id="{{ $invitation->id }}">
                                ...
                                <td field-key='rejected_at'>{{ $invitation->rejected_at }}</td>
                                    ...
                                    <a href="{{ route('admin.invitations.send',[$invitation->id]) }}" class="btn btn-xs btn-warning">Send</a>   
                                    @endif
...
@section('javascript') 
    <script>
        @can('invitation_delete')
            window.route_mass_crud_entries_destroy = '{{ route('admin.invitations.mass_destroy') }}';
        @endcan
