...
<div class="card-body">
    <div class="row">
        <div class="col-md-2">
            <img src="/storage/images/avatars/{{ $comment->user->avatar }}" class="img rounded-circle img-fluid" />
            <p class="text-secondary text-center">{{ $comment->created_at->diffForHumans() }}</p>
        </div>
        <div class="col-md-10">
            <p>
            <a class="float-left" href="/student_profile/{{$comment->user->id}}"><strong>{{$comment->user->name}}</strong></a>


                @if (!empty($thread->solution))
                    @if ($thread->solution == $comment->id)
                        <button class="mb-2 btn btn-success float-right font-weight-bold">Solution</button> 
                    @endif
                @else
                    @if (auth()->check())
                        @if (auth()->user()->id == $thread->user_id)
                            <form action="{{route('markAsSolution')}}" method="POST">
                                ...
                                <input class="mb-2 btn btn-outline-success float-right font-weight-bold" type="submit" id="{{$comment->id}}" value="Mark As a Solution">
                            </form>
                            {{-- <div  class="btn btn-outline-success float-right font-weight-bold" onclick="markAsSolution('{{$thread->id}}','{{$comment->id}}',this)">Mark As a Solution</div> --}}
                        @endif
                    @endif
                @endif
            ...
            {{-- Comment Actions --}}
            <p>
                @if ((Auth::user()->id == $comment->user_id) || (Auth::user()->type == 'admin')) {{-- Actions --}} 
                {{-- edit-form --}}
                ...
                <a class="float-right btn btn-sm text-white btn-dark" {{$comment->isLiked()?"liked":""}}" onclick="likeIt('{{$comment->id}}',this)" onclick="likeIt('{{$comment->id}}',this)"> <i class="fa fa-heart success"></i></a> --}}
    ...
    <div class="ml-5 card card-inner small mb-1" style="border-radius: 5%">
        <div class="card-body">
            <div class="row">
                <div class="col-md-2">
                    <img src="/storage/images/avatars/{{ $reply->user->avatar }}" class="img rounded-circle img-fluid" style="width: 50px; height 50px;"/>
                    <p class="text-secondary text-center">{{$reply->created_at->diffForHumans()}}</p>
                </div>
                <div class="col-md-10">
                    <p><a href="/student_profile/{{$reply->user->id}}"><strong>{{ $reply->user->name }}</strong></a></p>
                    <p>{!! $reply->body !!}</p>
